...
@plot='Профессиональный мошенник Рой Кортни едва верит своей удаче, когда знакомится в сети с богатой вдовой Бетти Маклиш.
 ...
 declare @rrc int=0;
 exec InsertMovie @name='Курьер', @release='5.12.2019', @country='США', @runningtime=105, @studioname='Signature Films', @genre='Боевик', 
@plot='На первый взгляд, она — всего лишь курьер на черном мотоцикле, стремительно пронзающем ночной Лондон. Но даже в огромном мегаполисе 
красотке удается ускользнуть от уличных камер, и ее имя невозможно найти ни в одной базе данных. Именно она, дерзкая и неуловимая, бросит 
вызов могущественному криминальному боссу. И ему лучше не знать, кто на самом деле эта взрывная брюнетка…', @rc=@rrc output;

 declare @rrc int=0;
 exec InsertMovie @name='Во все тяжкое', @release='12.11.2018', @country='США', @runningtime=90, @studioname='IM Global', @genre='Комедия', 
@plot='Профессор колледжа решает жить на полную катушку после того, как ему ставят серьезный диагноз. Отныне все запреты сняты, а тайные желания могут наконец осуществиться!', @rc=@rrc output;
