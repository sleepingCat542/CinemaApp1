...
--EXEC xp_cmdshell 'bcp.exe "SELECT  M.NAME[Name], C.NAME[Cinema], H.NAME[Hall], S.DATE[Date], TIME[Time], ROW, SEAT,  PRICE, UNICK_TICKET[Unick]  FROM USERS U INNER JOIN PURCHASE P ON U.ID=P.USER_ID INNER JOIN
--  TICKETS T ON P.ID=T.PURCHASE_ID INNER JOIN SESSION S ON S.ID=T.SESSION_ID INNER JOIN MOVIE M ON M.ID = S.MOVIE_ID 
--	INNER JOIN HALL H ON H.ID = S.HALL_ID INNER JOIN CINEMA C ON C.ID = H.CINEMA_ID FOR XML PATH(''Ticket''), ROOT(''Root'') " queryout "C:\Users\User\Desktop\1.xml" -d Cinema -w';
...
--  TICKETS T ON P.ID=T.PURCHASE_ID INNER JOIN SESSION S ON S.ID=T.SESSION_ID INNER JOIN MOVIE M ON M.ID = S.MOVIE_ID 
--	INNER JOIN HALL H ON H.ID = S.HALL_ID INNER JOIN CINEMA C ON C.ID = H.CINEMA_ID FOR XML PATH('Ticket'), ROOT('Root');
